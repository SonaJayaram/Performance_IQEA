...
# as of now hard coded to verify
...
def load_prompt_from_file(prompt_type):
    ...
    if source == "file":
        ...
        elif prompt_type == "performance_testcase_generation":
            prompt_file = os.path.join(config_folder, "performance_testcase_generation_prompt.txt")
        else:
...
def get_output_from_ai( final_prompt):
    response = ""
    try:
        response = client.chat.completions.create(model="qepracticekey",
                                              messages=[{"role": "user",
                                                         "content": final_prompt
                                                         }
                                                        ])
    except Exception as e:
        print(f"[ERROR] LLM call failed for chunk : {e}")
    print("### ai response ### ", response)

def get_queries_from_ai(prompt, formatted_summary):
    print("## going inside get_queries_from_ai")
    # model = "gpt-5-mini"
    model = "qepracticekey"
...
def save_response_to_jmetertestcase(markdown_text, test_collection, output_file="BlazeDemo"+generate_random_prefix()+".jmx"):
    if isinstance(markdown_text, list):
        markdown_text = "\n".join(str(line) for line in markdown_text)
    elif not isinstance(markdown_text, str):
        markdown_text = str(markdown_text)
    if source == "file":
        ...
        jmx_path = os.path.join(test_collection, output_file)
        with open(jmx_path, "w", encoding="utf-8") as f:
            f.write(markdown_text)

        print(f"✅ File saved successfully at: {jmx_path}")
        return jmx_path

def covert_response_to_testcases_single_sheet(markdown_text, test_collection, output_file="BlazeDemo"+generate_random_prefix()+".xlsx"):
