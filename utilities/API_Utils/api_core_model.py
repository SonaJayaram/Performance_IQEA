...
class Apicore:
    ...
    def validate_post_response(self, response, request_payload):
        # Try parsing safely to avoid crashing on HTML error pages or empty text
        try:
            response_json = response.json()
            print("validate_post_response-response", response_json)
        except Exception:
            print("validate_post_response-response (Non-JSON text)", response.text)
            response_json = {}
