...
def get_locust_output_from_ai(prompt: str) -> str:
    """Sends prompt to AI/Azure OpenAI engine specifically tailored for generating Python Locust scripts."""
    try:
        # Use your existing LLM invocation logic (e.g., swagger_utils or Azure OpenAI client)
        from utilities.API_Utils import swaggerhub as swagger_utils

        raw_response = swagger_utils.get_queries_from_ai_updated(prompt)

        # Clean up code blocks and markdown formatting
        clean_code = str(raw_response or "").replace("```python", "").replace("```", "").strip()
        return clean_code
    except Exception as e:
        print(f"Error in get_locust_output_from_ai: {e}")
        return ""